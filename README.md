...
- **Branch and directory**: the local branch and working directory for this workstream
...
Each Wing has a launch profile: your preferred editor (VS Code, Cursor), terminal (Ghostty, iTerm2, Warp), and optionally a tmux layout. Launching a Space opens your editor at the right directory and starts (or reattaches to) a named tmux session with `claude` pre-loaded with the Space's todos, notes, PRs, and branch already in context. If you've worked in the Space before, the prior Claude session is resumed automatically.
...
**Data.** All state is stored as plain JSON in `~/.atrium/`: Wings, Spaces, config, and watched PRs. Easy to inspect, back up, or reset.
...
2. Run `npx tsc --noEmit` and `npx eslint src/` before pushing.
3. Run `npx prettier --write` on any files you touched.
